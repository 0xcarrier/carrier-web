...
import SVGSolflareLogo from '../../../assets/svgs/wallet-logos/solflare.svg';
...
export function useSolflare(): WalletInterface {
  const {
    select,
    connect: _connect,
  ...
  const wallet = useMemo(() => {
    return wallets.find((item) => item.adapter.name === SolflareWalletName);
  }, [wallets]);
