...
import talismanIcon from '../../../assets/svgs/wallet-logos/talisman.svg';
...
const walletName = 'Talisman';

export function useTalisman(): WalletInterface {
  ...
  const isInstalled = useCallback(async () => {
    ...
    return allInjected.findIndex((item) => item.name === 'talisman') !== -1;
  }, []);

  const install = useCallback(async () => {
    window.open('https://www.talisman.xyz/');
    ...
    async (options: { chainId: CarrierChainId; silence?: boolean; selectedAccount?: string }) => {
      ...
      try {
        ...
        try {
          ...
          detectedInjected = allInjected.find((item) => item.name === 'talisman');
        } catch (e) {
        ...
        const { accounts, addressPrefix, genesisHash } = await getAccounts({ chainId, walletName: 'talisman' });
        ...
        if (accounts && accounts.length && addressPrefix != null) {
          ...
          console.log('selectedAccount', accountAddress, accounts, selectedAccount);
        ...
        const sub = await web3AccountsSubscribe(
          ...
          { extensions: ['talisman'], genesisHash, ss58Format: addressPrefix },
        );
  ...
  const getAccountList = useCallback(async (options: { chainId: CarrierChainId }) => {
    ...
    const { accounts } = await getAccounts({ chainId, walletName: 'talisman' });
