...
import subWalletJsIcon from '../../../assets/svgs/wallet-logos/subwallet-js.svg';
...
export function useSubWallet(): WalletInterface {
  ...
  const isInstalled = useCallback(async () => {
    ...
    return allInjected.findIndex((item) => item.name === 'subwallet-js') !== -1;
  }, []);

  const install = useCallback(async () => {
    window.open('https://www.subwallet.app/');
    ...
    async (options: { chainId: CarrierChainId; silence?: boolean; selectedAccount?: string }) => {
      ...
      try {
        ...
        try {
          ...
          detectedInjected = allInjected.find((item) => item.name === 'subwallet-js');
        } catch (e) {
        ...
        const { accounts, addressPrefix, genesisHash } = await getAccounts({ chainId, walletName: 'subwallet-js' });
        ...
        const sub = await web3AccountsSubscribe(
          ...
          { extensions: ['subwallet-js'], genesisHash, ss58Format: addressPrefix },
        );
  ...
  const getAccountList = useCallback(async (options: { chainId: CarrierChainId }) => {
    ...
    const { accounts } = await getAccounts({ chainId, walletName: 'subwallet-js' });
