...
export const AmountInput: React.SFC<Props> = ({ sourceToken, disabled, amountData, onAmountChanged }) => {
  ...
  return (
    <div className={styleAmountWrapper}>
      <input
        ...
        placeholder="0.00"
        value={inputValue || ''}
        disabled={!sourceToken || disabled}
        onChange={(event) => {
          if (sourceToken) {
            const input = validateTokenInput(amountData.transferAmountString, event.target.value, sourceToken.decimals);
