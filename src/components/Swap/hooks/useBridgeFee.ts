...
export function useBridgeFee(options: {
  ...
  return useMemo((): BridgeFee => {
    const relayerFee =
      isUsingRelayer && providerFeeData.data && providerFeeData.data.relayable
        ? providerFeeData.data.relayerUsdString || '0'
        : '0';
    const totalFees =
      feeData.data && feeData.data.sourceFee && feeData.data.targetFee
        ? feeData.data.sourceFee.plus(feeData.data.targetFee).plus(BigNumber(relayerFee))
        : undefined;
  ...
  }, [feeData, providerFeeData, isUsingRelayer]);
}
